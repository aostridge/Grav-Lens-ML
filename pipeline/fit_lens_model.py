...
from config import OBSERVATIONS_DIR, PIPELINE_DIR, LENSMODEL_PATH, MODELS_DIR
...
CHI_SQR_ACCURACY = 8
TOLERANCE        = 0.0005
TOL_COUNT        = 10   # matches paper §3.4: "10 consecutive attempts"
EXPIRE_LIMIT     = 10   # matches paper §4.2: "timed out 10 consecutive times"
BINARY_LENGTH    = 10
BINARY_ONES      = 4
...
# =============================================================================
# Paths & run configuration  (edit pipeline/config.py to change directory layout)
# =============================================================================
FILE_PATH     = OBSERVATIONS_DIR
FILE_NAME     = 'quad_lens_observations.txt'
PIPELINE_PATH = PIPELINE_DIR  # pipeline/ directory — location of lens_model.* templates
...
# Rotated coordinates are the real-data equivalent of NEW_COORD_COLS from
# columns.py: training data is generated at ellipticity angle 0 (no rotation
# needed), while real data is recentred on the diagonal intersection then
# rotated to remove the ellipticity angle — producing the same canonical
# orientation. The column names differ between the two pipelines but the
# values are geometrically equivalent.
# =============================================================================
...
def optimize_model(row, output_path, results_dir_name, acc, tolerance, expire_limit,
    ...
    # Read templates from pipeline/ — single canonical source shared with generate_training_data.py.
    # Written copies go to output_path each iteration; the pipeline/ originals are never modified.
    input_file = pd.read_csv(
        PIPELINE_PATH / 'lens_model.input', header=None, skip_blank_lines=True, comment='#', dtype=object
    )
    start_file = pd.read_csv(
        PIPELINE_PATH / 'lens_model.start', header=None, skip_blank_lines=True, comment='#', dtype=object
    )
    data_file = pd.read_csv(
        PIPELINE_PATH / 'lens_model.dat', header=None, skip_blank_lines=True, comment='#', dtype=object
    )
    ...
    while not finished:
        ...
        iteration += 1
        print(f"  [{row['Image']}] iter={iteration} | mask={param_mask} | first_pass={is_first_pass} | stall={stall_count} | timeouts={timeout_count} | angle_rotated={angle_rotated}", flush=True)
        ...
        try:
            ...
            if not is_first_pass:
                ...
                if chi_sqr < acc:
                    ...
                    if not angle_rotated:
                        print(f"  [{row['Image']}] Stall limit reached — rotating angle by 90deg", flush=True)
            ...
            if timeout_count >= expire_limit:
                ...
                if not angle_rotated:
                    print(f"  [{row['Image']}] Expire limit reached — rotating angle by 90deg", flush=True)
                    is_first_pass   = True
                    df.at[row.name, 'Ellipticity Angle'] += math.pi / 2
                    angle_rotated   = True
                    timeout_count   = 0
...
checkpoint_path = FILE_PATH / results_dir_name / 'optimisation_results.csv'

# Resume support: if a checkpoint CSV already exists, restore completed rows and skip them.
completed = set()
if checkpoint_path.exists():
    prev_df = pd.read_csv(checkpoint_path)
    completed = set(prev_df.index[prev_df['Chi Sqr'].notna()])
    df.update(prev_df)
    print(f"[Resume] Found checkpoint — skipping {len(completed)} already-completed observation(s).", flush=True)

for index, row in df.iterrows():
    print(f"\n{'='*60}", flush=True)
    print(f"Test {index + 1}/{len(df)}: {row['Image']}", flush=True)
    print(f"{'='*60}", flush=True)

    if index in completed:
        print(f"  Skipping {row['Image']} (already in checkpoint).", flush=True)
        continue

    # --- Pre-optimisation snapshot ---
    # Write the NN predictions and initial model string to disk before the optimiser
    # starts so they survive a crash (they are otherwise only held in memory).
    obs_folder = FILE_PATH / results_dir_name / row['Image']
    create_folder_if_not_exists(obs_folder)
    initial_model_str = (
        f'alpha {row["Predicted Mass"]:.6e} '
        f'{row[f"Ellipse Centre RA{IMAGE_RECENTRE_SUFFIX}"]:.6e} '
        f'{row[f"Ellipse Centre Dec{IMAGE_RECENTRE_SUFFIX}"]:.6e} '
        f'{row["Predicted Ellip"]:.6e} '
        f'{math.degrees(row["Ellipticity Angle"]):.6f} '
        f'0. 0. 0. 0. 1.'
    )
    with open(obs_folder / 'pre_optimisation.txt', 'w') as f:
        f.write(
            f"Image:              {row['Image']}\n"
            f"Predicted Mass:     {row['Predicted Mass']:.6e}\n"
            f"Predicted Ellip:    {row['Predicted Ellip']:.6e}\n"
            f"Ellipticity Angle:  {math.degrees(row['Ellipticity Angle']):.6f} deg\n"
            f"Ellipse Centre RA:  {row[f'Ellipse Centre RA{IMAGE_RECENTRE_SUFFIX}']:.6e} arcsec\n"
            f"Ellipse Centre Dec: {row[f'Ellipse Centre Dec{IMAGE_RECENTRE_SUFFIX}']:.6e} arcsec\n"
            f"Initial model:      {initial_model_str}\n"
        )
    ...
    if (index + 1) % CHECKPOINT_EVERY == 0:
        ...
        print(f"[Checkpoint] Saved after {index + 1} observations.", flush=True)
