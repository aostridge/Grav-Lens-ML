...
from config import FILE_NAME, OVERWRITE_PREDICTIONS, PREDICTION_MODE, SOURCES_DIR, MODELS_DIR
...
# =============================================================================
# Paths  (edit pipeline/config.py to change directory layout)
# =============================================================================
MODEL_MASS  = 'model_mass_parameter_paper.keras'
MODEL_ELLIP = 'model_ellipticity_paper.keras'
MODEL_BOTH  = 'model_mass_ellipticity.keras'  # no pretrained "_paper" variant is shipped —
                                               # the paper's final architecture is 'separate' (Table 1)
...
# Skip check
# =============================================================================
_predictions_out = MODELS_DIR / f'{prediction_file}.parquet'
...
if PREDICTION_MODE == 'separate':
    mass_loaded_model  = tf.keras.models.load_model(MODELS_DIR / MODEL_MASS)
    ellip_loaded_model = tf.keras.models.load_model(MODELS_DIR / MODEL_ELLIP)
    mass_loaded_model.training  = False
    ellip_loaded_model.training = False
else:
    combined_loaded_model = tf.keras.models.load_model(MODELS_DIR / MODEL_BOTH)
...
data = pd.read_parquet(
    SOURCES_DIR / f'{FILE_NAME}_features.parquet',
    columns=load_cols,
)
...
if save_predictions:
    ...
    output_df.to_parquet(MODELS_DIR / f'{prediction_file}.parquet', index=False)
...
plt.tight_layout()
if save_ellip_plot:
    plt.savefig(MODELS_DIR / 'plots' / f'{ellip_violin_plot_name}.png')
plt.close()
...
if save_mass_plot:
    plt.savefig(MODELS_DIR / 'plots' / f'{mass_violin_plot_name}.png')
plt.close()
